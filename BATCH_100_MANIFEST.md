...
> **Status (2026-07-02):** Historical task ledger for Batch-100. Key File paths corrected to match current repo layout (`backend/internal/{domain}/`, `backend/cmd/`). Task completion status aligned with `FINAL_COMPLETION_MANIFEST.md`.
...
| B005 | WS    | done    | Server-event envelope (type/payload/request_id)     | `backend/internal/realtime/protocol.go`                                       |
...
| B008 | WS    | done    | WS error envelope with `request_id`                 | `backend/internal/realtime/protocol.go`                                       |
...
| B011 | WS    | done    | Typing indicator WS event (no DB persistence)       | `backend/internal/realtime/protocol.go`                                       |
...
| A003 | Core  | done    | Health endpoint `/health`                           | `backend/internal/server/server.go`                                            |
...
| A006 | Core  | done    | Users table + repository                            | `backend/internal/user/repository.go`                                             |
| A007 | Core  | done    | Password hashing (bcrypt)                           | `backend/internal/auth/password.go`                                         |
| A008 | Core  | done    | Register API (`POST /api/auth/register`)            | `backend/internal/auth/service.go`                                              |
| A009 | Core  | done    | Login API (`POST /api/auth/login`)                  | `backend/internal/auth/service.go`                                              |
| A010 | Core  | done    | JWT middleware                                      | `backend/internal/auth/service.go`                                       |
| A011 | Core  | done    | Refresh token skeleton                              | `backend/internal/auth/service.go`                                              |
| A012 | Core  | done    | Devices table + repository                          | `backend/internal/device/repository.go`                                           |
| A013 | Core  | done    | Conversations table + repository                    | `backend/internal/conversation/repository.go`                                     |
| A014 | Core  | done    | Conversation members table                          | `backend/internal/conversation/members.go`                                  |
| A015 | Core  | done    | Messages table + repository                         | `backend/internal/message/repository.go`                                          |
| A016 | Core  | done    | Create DM API (`POST /api/conversations/direct`)        | `backend/internal/conversation/handler.go`                                      |
| A017 | Core  | done    | Create group API (`POST /api/conversations/groups`)  | `backend/internal/conversation/handler.go`                                      |
| A018 | Core  | done    | Send message REST (`POST /api/conversations/:id/messages`) | `backend/internal/message/handler.go`                                    |
| A019 | Core  | done    | Message history pagination API                      | `backend/internal/message/handler.go`                                           |
| A020 | Core  | done    | Unified error envelope                              | `backend/internal/apierror/error.go`                                            |
| A021 | Core  | done    | OpenAPI skeleton                                    | `docs/openapi.yaml`                                                         |
| A022 | Core  | done    | Seed script                                         | `scripts/seed.sh`, `backend/cmd/seed/main.go`                                                           |
| C001 | Sync  | done    | `latest_seq` on conversation                        | `backend/migrations/`, `backend/internal/conversation/repository.go`              |
| C002 | Sync  | done    | Seq allocation in transaction                       | `backend/internal/message/repository.go`                                          |
| C003 | Sync  | done    | Conversation list API with recency sort             | `backend/internal/conversation/handler.go`                                      |
...
| C006 | Sync  | done    | Sync endpoint (`POST /api/sync`)                     | `backend/internal/sync/handler.go`                                              |
| C007 | Sync  | done    | Per-device sync cursor (`device_sync_cursors`)      | `backend/internal/sync/cursor.go`                                           |
| C008 | Sync  | done    | Message edit API (`PATCH /api/conversations/:id/messages/:message_id`)        | `backend/internal/message/handler.go`                                           |
| C009 | Sync  | done    | Message recall API (`POST /api/conversations/:id/messages/:message_id/recall`)| `backend/internal/message/handler.go`                                           |
| C010 | Sync  | done    | Pinned message API                                  | `backend/internal/message/handler.go`                                           |
...
| D003 | Rel   | done    | Message ACK API + WS event                          | `backend/internal/delivery/handler.go`                                               |
...
| D007 | Rel   | done    | Outbox publisher worker (SKIP LOCKED)               | `backend/internal/outbox/publisher.go`                                         |
| D008 | Rel   | partial | Dead-letter skeleton                                | `backend/internal/outbox/dlq_handler.go`                                            |
...
| E001 | Group | done    | Group member roles owner/admin/member               | `backend/internal/conversation/authz.go`                                    |
| E002 | Group | done    | Invite/kick/leave group APIs                        | `backend/internal/conversation/handler.go`                                             |
| E003 | Group | done    | Channel data model                                  | `backend/migrations/`                                                       |
| E004 | Group | done    | Channel subscribe/unsubscribe APIs                  | `backend/internal/conversation/handler.go`                                           |
| E005 | Group | done    | Channel publish permission (owner/admin only)       | `backend/internal/conversation/handler.go`                                           |
...
| E008 | Group | done    | Fanout worker skeleton                              | `backend/internal/push/worker.go`                                         |
...
| F001 | Infra | done    | Redis client                                        | `backend/internal/redisx/client.go`                                           |
| F002 | Infra | done    | Redis rate limiter                                  | `backend/internal/rate_limit/middleware.go`                                  |
| F003 | Infra | done    | Redis presence TTL                                  | `backend/internal/presence/store.go`                                        |
| F004 | Infra | done    | Redis conversation list cache (30 s TTL)            | `backend/internal/cache/conversation.go`                                        |
| F005 | Infra | done    | Kafka client                                        | `backend/internal/eventbus/kafka.go`                                              |
| F006 | Infra | done    | `message.created` publish                           | `backend/internal/outbox/publisher.go`                                           |
...
| F009 | Infra | partial | MQ lag metrics (Kafka consumer lag)                 | `backend/internal/metrics/metrics.go`                                         |
| F010 | Infra | done    | Cache consistency ADR                               | `docs/adr/0005-cache-consistency.md`                                        |
| G001 | Media | done    | MinIO presign upload URL                            | `backend/internal/media/handler.go`                                             |
| G002 | Media | done    | MinIO presign download URL                          | `backend/internal/media/handler.go`                                             |
| G003 | Media | done    | Attachment metadata table                           | `backend/migrations/`                                                       |
| G004 | Media | done    | Attachment message type                             | `backend/internal/message/repository.go`                                          |
| G005 | Media | done    | Presigned download URL endpoint                     | `backend/internal/media/handler.go`                                             |
| G006 | Media | done    | Message indexing worker on `message.created`        | `backend/internal/worker/handlers.go`                                   |
| G007 | Media | done    | Full-text search API (`GET /api/search/messages`)   | `backend/internal/search/handler.go`                                            |
| G008 | Media | done    | Search scoped to member conversations               | `backend/internal/search/handler.go`                                            |
...
| H001 | Sec   | done    | Redis rate limit middleware (login)                 | `backend/internal/rate_limit/middleware.go`                                  |
| H002 | Sec   | done    | Rate limit on conv send                             | `backend/internal/rate_limit/middleware.go`                                  |
| H003 | Sec   | done    | Rate limit on register                              | `backend/internal/rate_limit/middleware.go`                                  |
| H004 | Sec   | done    | Audit log table + service                           | `backend/internal/audit/`                                                   |
| H005 | Sec   | done    | Login audit logging                                 | `backend/internal/auth/service.go`                                              |
| H006 | Sec   | done    | Recall audit logging                                | `backend/internal/message/handler.go`                                           |
| H007 | Sec   | done    | Input sanitisation middleware                       | `backend/internal/middleware/sanitize.go`                                   |
...
| I001 | Obs   | done    | Structured JSON logs + `X-Trace-ID` middleware      | `backend/internal/apierror/trace.go`                                      |
| I002 | Obs   | done    | Request logger middleware                           | `backend/internal/server/server.go`                                     |
| I003 | Obs   | done    | Prometheus `/metrics`                               | `backend/internal/metrics/metrics.go`                                    |
| I004 | Obs   | done    | WS connection gauge metric                          | `backend/internal/metrics/metrics.go`                                            |
| I005 | Obs   | done    | Message send latency histogram                      | `backend/internal/metrics/metrics.go`                                      |
...
| J002 | FE    | done    | Login page                                          | `frontend/src/pages/LoginPage.tsx`                                              |
| J003 | FE    | done    | Conversation list sidebar                           | `frontend/src/pages/ChatPage.tsx`                              |
| J004 | FE    | done    | Chat window with message history                    | `frontend/src/pages/ChatPage.tsx`                                    |
| J005 | FE    | done    | Message history pagination                          | `frontend/src/pages/ChatPage.tsx`                                    |
| J006 | FE    | done    | WS reconnect logic                                  | `frontend/src/pages/ChatPage.tsx` (WS client)                                        |
| J007 | FE    | done    | Optimistic send UI                                  | `frontend/src/pages/ChatPage.tsx`                                    |
| J008 | FE    | done    | Attachment upload UI                                | `frontend/src/components/AttachmentUpload.tsx`                              |
| J009 | FE    | done    | Search UI in sidebar                                | `frontend/src/pages/ChatPage.tsx`                                     |
...
| SB03 | Backlog   | done    | Emoji reactions skeleton                     | `backend/internal/reaction/handler.go`           |
...
| SB06 | Backlog   | done    | Admin panel API skeleton                     | `backend/internal/admin/handler.go`               |
| SB07 | Backlog   | done    | Conversation archive/mute API                | `backend/internal/conversation/handler.go`        |
| SB08 | Backlog   | partial | Dark mode CSS                                | `frontend/src/styles.css`               |
| SB09 | Backlog   | done    | Message forward API skeleton                 | `backend/internal/message/handler.go`             |
| SB10 | Backlog   | done    | User profile update API                      | `backend/internal/user/profile.go`                |
...
| ST04 | Stretch | done    | Conversation export API (JSON/CSV)                  | `backend/internal/export/handler.go`               |
...
| ST08 | Stretch | partial | Rate limit per conversation                         | `backend/internal/rate_limit/middleware.go`     |
...
| X009 | Ext   | done    | Payments ledger ADR (canonical 0019)           | `docs/adr/0019-payment-ledger.md`            |
