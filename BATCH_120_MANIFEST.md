...
|------|---------|---------|--------------------------------------------------------------------------------|-----------|
| T001 | backend | done    | User registration, login, JWT access + refresh tokens                          | `backend/internal/auth/handler.go`, `backend/internal/auth/service.go` |
...
| T003 | backend | done    | Group create/invite/kick/leave with role ACL (owner/admin/member)              | `backend/internal/group/handler.go`, `backend/internal/group/service.go` |
| T004 | backend | done    | Channel create/subscribe/publish with publish-permission guard                 | `backend/internal/channel/handler.go`, `backend/internal/channel/service.go` |
...
| T007 | backend | done    | Per-device sync cursor: GET /sync returns messages after device cursor          | `backend/internal/message/handler.go` (sync handler), migration `00007` |
| T008 | backend | done    | Delivery ACK REST + WebSocket ACK frame; delivery state machine (sent→delivered→read) | `backend/internal/delivery/handler.go`, `backend/internal/realtime/hub.go` |
| T009 | backend | done    | Outbox pattern: enqueue on send, SKIP LOCKED worker drain, Kafka publish        | `backend/internal/outbox/`, `backend/worker/` |
| T010 | backend | done    | Dead-letter queue (DLQ) skeleton: failed events written to `dead_letter` table | `backend/migrations/00006_dead_letter.sql`, `backend/internal/outbox/worker.go` |
...
| T013 | backend | done    | Prometheus metrics: HTTP duration, WS connections, message counters             | `backend/internal/metrics/`, `backend/api/main.go` |
| T014 | backend | done    | OpenTelemetry trace_id injected into request context + structured log           | `backend/internal/tracing/`, `backend/api/main.go` |
...
| T016 | backend | done    | PostgreSQL full-text search API: GET /search?q= across messages                 | `backend/internal/search/`, `backend/migrations/00008_message_search_index.sql` |
| T017 | backend | done    | Social features: pin conversation, block user, report user, mute conversation   | `backend/internal/social/`, `backend/migrations/00009_social.sql` |
...
| T069 | ops   | planned | GitHub Actions deploy workflow: build image → push to GHCR → rolling restart     | `.github/workflows/deploy.yml` (planned) |
| T070 | ops   | planned | Sentry error tracking integration in backend and frontend                         | `backend/internal/telemetry/sentry.go` (planned) |
...
| T072 | docs  | done    | ADR 0002–0015: message sequence, cache/MQ, fanout, WS routing, E2EE, ads, payments, mobile, desktop | `docs/adr/` |
...
| T097 | tests   | done    | Social unit tests: pin, block, mute, report                                         | `backend/internal/social/*_test.go` |
...
| T107 | tests   | planned | Reaction API tests: add, remove, list reactions via REST                            | `backend/internal/reaction/*_test.go` (planned) |
| T108 | tests   | planned | Thread reply API tests: create reply, fetch thread                                  | `backend/internal/message/*_test.go` (planned) |
...
| T117 | ext     | planned | E2EE: Signal Protocol double ratchet (client-side, server stores ciphertext only)  | `docs/encryption-prototype.md`, ADR `0010`–`0011` |
| T118 | ext     | planned | Payment features: subscribe to channel, pay-per-message                             | `docs/payments-prototype.md`, ADR `0019` |
| T119 | ext     | planned | Ads targeting: CPM bidding, frequency cap, campaign budget                          | `docs/ads-prototype.md`, ADR `0020` |
| T120 | ext     | planned | Recommendation engine: friend suggestions, channel discovery                        | `docs/recommendation-prototype.md`, ADR `0021` |
